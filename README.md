...
1. 按 `docs/BACKEND.md` 创建数据库资源并部署 `cloud-hop` Edge Function。
...
## 页面布局
主页面以居中的大画面为主，分数、场景和有效道具浮在画面内，排行榜在下方。右上角「玩法说明」进入独立页面（[打开说明](https://cloud-hop.vercel.app/#/guide)），包含玩法、规则、道具和场景介绍。打开说明暂停对局，返回后点击继续。
